...
// ParseTBTCSystemABI parses the ABI
func ParseTBTCSystemABI() (*abi.ABI, error) {
	parsed, err := abi.JSON(strings.NewReader(TBTCSystemABI))
	if err != nil {
		return nil, err
	}
...
func (_TBTCSystem *TBTCSystemFilterer) TryParseLog(log types.Log) (eventName string, event interface{}, ok bool, err error) {
	eventName, ok, err = _TBTCSystem.contract.LogEventName(log)
	if err != nil || !ok {
		return "", nil, false, err
	}

	switch eventName {
	case "AllowNewDepositsUpdated":
		event, err = _TBTCSystem.ParseAllowNewDepositsUpdated(log)
	case "CollateralizationThresholdsUpdateStarted":
		event, err = _TBTCSystem.ParseCollateralizationThresholdsUpdateStarted(log)
	case "CollateralizationThresholdsUpdated":
		event, err = _TBTCSystem.ParseCollateralizationThresholdsUpdated(log)
	case "CourtesyCalled":
		event, err = _TBTCSystem.ParseCourtesyCalled(log)
	case "Created":
		event, err = _TBTCSystem.ParseCreated(log)
	case "EthBtcPriceFeedAdded":
		event, err = _TBTCSystem.ParseEthBtcPriceFeedAdded(log)
	case "EthBtcPriceFeedAdditionStarted":
		event, err = _TBTCSystem.ParseEthBtcPriceFeedAdditionStarted(log)
	case "ExitedCourtesyCall":
		event, err = _TBTCSystem.ParseExitedCourtesyCall(log)
	case "FraudDuringSetup":
		event, err = _TBTCSystem.ParseFraudDuringSetup(log)
	case "Funded":
		event, err = _TBTCSystem.ParseFunded(log)
	case "FunderAbortRequested":
		event, err = _TBTCSystem.ParseFunderAbortRequested(log)
	case "GotRedemptionSignature":
		event, err = _TBTCSystem.ParseGotRedemptionSignature(log)
	case "KeepFactoriesUpdateStarted":
		event, err = _TBTCSystem.ParseKeepFactoriesUpdateStarted(log)
	case "KeepFactoriesUpdated":
		event, err = _TBTCSystem.ParseKeepFactoriesUpdated(log)
	case "Liquidated":
		event, err = _TBTCSystem.ParseLiquidated(log)
	case "LotSizesUpdateStarted":
		event, err = _TBTCSystem.ParseLotSizesUpdateStarted(log)
	case "LotSizesUpdated":
		event, err = _TBTCSystem.ParseLotSizesUpdated(log)
	case "OwnershipTransferred":
		event, err = _TBTCSystem.ParseOwnershipTransferred(log)
	case "Redeemed":
		event, err = _TBTCSystem.ParseRedeemed(log)
	case "RedemptionRequested":
		event, err = _TBTCSystem.ParseRedemptionRequested(log)
	case "RegisteredPubkey":
		event, err = _TBTCSystem.ParseRegisteredPubkey(log)
	case "SetupFailed":
		event, err = _TBTCSystem.ParseSetupFailed(log)
	case "SignerFeeDivisorUpdateStarted":
		event, err = _TBTCSystem.ParseSignerFeeDivisorUpdateStarted(log)
	case "SignerFeeDivisorUpdated":
		event, err = _TBTCSystem.ParseSignerFeeDivisorUpdated(log)
	case "StartedLiquidation":
		event, err = _TBTCSystem.ParseStartedLiquidation(log)
	}
	if err != nil {
		return "", nil, false, err
	}
