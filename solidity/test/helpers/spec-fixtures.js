...
const redemptionPaymentColumns = redemptionPaymentTable
  .getHeadRows()[0]
  .map((_) => _.getText())
...
function redemptionPaymentFixturesFor(lotSize, signerFee) {
  ...
  function lookupValue(valueVariable) {
    ...
    switch (variableMatch[0]) {
      ...
      case "{tbtc-lot-size}":
        return lotSize
      case "{tbtc-lot-size-less-signer-fee}":
        return lotSize.sub(signerFee)
      case "{btc-lot-size}":
        return new BN(0) // Nothing due inside the system.
      default:
        return new BN(0)
    }
  }
  ...
  return redemptionPaymentTable.getBodyRows().flatMap((row, index) => {
      ...
      switch (columnName) {
        ...
        case "Disbursal Amounts":
          ...
          if (!fixture.redemptionShouldRevert) {
            const disbursalList = /** @type {DListItems} */ (
          ...
          continue
        default:
          const camelCaseName = columnName
    ...
    fixture.description =
      (fixture.preTerm ? "Pre-term" : "At-term ") + fixture.description
    ...
    if (!fixture.preTerm) {
      ...
      // different.
      const courtesyCallCopy = Object.assign(
        {
          courtesyCall: true,
          description: fixture.description.replace("At-term ", "Courtesy"),
        },
        fixture
      )
