...
const System = {
  ...
  setAndApproveRedemptionBalance: async ({
    TestTBTCToken,
    deposit,
    ...
    await TestTBTCToken.approve(deposit.address, redemptionRequirement, {
      from: opener,
    })
  },
  setAndApproveVendingMachineRedemptionBalance: async ({
    VendingMachine,
    TestTBTCToken,
    redemptionRequirement,
  }) => {
    ...
    await TestTBTCToken.approve(VendingMachine.address, redemptionRequirement, {
      from: opener,
    })
  },
  setAndApproveLiquidationBalance: async ({ TestTBTCToken, deposit }) => {
    ...
    await TestTBTCToken.approve(deposit.address, requirement, {
      from: liquidator,
    })
  },
  setWellCollateralized: async ({
    deposit,
    ECDSAKeepStub,
    mockSatWeiPriceFeed,
  }) => {
    ...
    const wellCollateralized = bondAmount.div(
      lotSize.mul(initial).div(new BN(100))
    )
    await mockSatWeiPriceFeed.setPrice(wellCollateralized)
  },
  setUndercollateralized: async ({
    deposit,
    ECDSAKeepStub,
    mockSatWeiPriceFeed,
  }) => {
    ...
    const undercollateralized = bondAmount.div(
      lotSize.mul(under.sub(new BN(1))).div(new BN(100))
    )
    await mockSatWeiPriceFeed.setPrice(undercollateralized)
  },
  setSeverelyUndercollateralized: async ({
    deposit,
    ECDSAKeepStub,
    mockSatWeiPriceFeed,
  }) => {
    ...
    const severelyUndercollateralized = bondAmount.div(
      lotSize.mul(severe.sub(new BN(1))).div(new BN(100))
    )
      ...
      awaitingSignerSetup: {
        transition: async ({
          ...
          return {
            ...
            tx: DepositFactory.createDeposit(availableLotSizes[0], {
              value: feeEstimate,
              from: opener,
            }),
            resolveDeposit: ({ Deposit }, receipt) => {
              const depositAddress = receipt.logs.find(
                (_) => _.event == "DepositCloneCreated"
              ).args.depositCloneAddress
        ...
        expect: async (
          _,
          receipt,
          { TBTCDepositToken, ECDSAKeepStub, deposit }
        ) => {
  ...
  awaitingSignerSetup: {
    ...
    failNext: {
      ...
      "signerSetupFailure when no bond is available": {
        ...
        expectError: async (_, error) => {
          expect(error.message).to.match(/No funds received, unexpected/)
        },
      },
    },
  },
    ...
    next: {
      active: {
        ...
        expect: async (_, receipt, { deposit }) => {
          ...
          expect(await deposit.currentState()).to.eq.BN(System.States.ACTIVE)
        },
      },
      failedSetup_timeout: {
    ...
    next: {
      minted: {
        ...
        expect: async (
          _,
          receipt,
          { deposit, VendingMachine, lotSize, signerFee }
        ) => {
    ...
    next: {
      redeemed: {
        ...
        expect: async (_, receipt, { deposit }) => {
          ...
          expect(await deposit.currentState()).to.eq.BN(System.States.REDEEMED)
        },
      },
      liquidated: {
        transition: async (state) => {
