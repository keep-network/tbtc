...
const StateRunner = {
  ...
  resolveResults: async (initialState, transitionResult) => {
    const receipt = await transitionResult.tx.then((_) =>
      resolveAllLogs(_.receipt, initialState)
    )
    const resolved = {}
    for (const [property, resolver] of Object.entries(transitionResult)) {
      if (property.startsWith("resolve")) {
        ...
        resolved[resolvedProperty] = await resolver(initialState, receipt)
      }
    }
    ...
    if (expectedSuccesses.length) {
      describe(`should transition from ${stateDefinition.name}`, () => {
        before(async () => {
          ...
          resolvedInitialState = await StateRunner.resolveDependencies(
            baseState,
        ...
        for (const [
          nextStateName,
          { transition, after, expect },
        ] of Object.entries(stateDefinition.next)) {
          it(`to ${nextStateName}`, async () => {
            if (after) {
              await increaseTime(
                (await after(resolvedInitialState)).add(new BN(1))
              )
            }
            const transitionResult = await transition(resolvedInitialState)
            const receipt = await transitionResult.tx.then((_) =>
              resolveAllLogs(_.receipt, resolvedInitialState)
            )
            ...
            if (expect) {
              await expect(
                ...
                receipt,
                await StateRunner.resolveResults(
                  resolvedInitialState,
                  transitionResult
                )
              )
  ...
  advanceToState: async (baseStatePromise, stateDefinition, nextStateName) => {
    ...
    const resolvedInitialState = await StateRunner.resolveDependencies(
      baseState,
      stateDefinition.dependencies
    )
    ...
    const resolved = await StateRunner.resolveResults(
      resolvedInitialState,
      transitionResult
    )
    return resolved
   ...
   *         in the state path.
   */
  runStatePath: (stateDefinitions, baseState, firstStateName, ...path) => {
    return path.concat([null]).reduce(
      ({ currentState, definition }, nextStateName) => {
        ...
        if (nextStateName !== null) {
          return {
            currentState: stateTestsPromise.then(() =>
              StateRunner.advanceToState(
                currentState,
                definition,
                nextStateName
              )
