...
describe("Integration -- black-swan", async function () {
  ...
  it("Liquidate Deposits", async () => {
    ...
    for (let i = 0; i < deposits.length; i++) {
      const requirement = await deposits[
        i
      ].getOwnerRedemptionTbtcRequirement.call(depositInitiator)
      ...
      if (available.gte(totalRequirement)) {
        const tx0 = await tbtcToken.approve(
          deposits[i].address,
          totalRequirement,
          { from: depositInitiator }
        )
        const tx1 = await deposits[i].purchaseSignerBondsAtAuction({
          from: depositInitiator,
        })
        const tx2 = await deposits[i].withdrawFunds({ from: depositInitiator })
        const gasUSed = new BN(tx0.receipt.cumulativeGasUsed).add(
          new BN(tx1.receipt.cumulativeGasUsed).add(
            new BN(tx2.receipt.cumulativeGasUsed)
          )
        )
    ...
    expect(
      liquidatedIndex,
      "Should be able to liquidate all but 1 deposit"
    ).to.eq.BN(deposits.length - 1)
    expect(new BN(initialBalance).add(new BN(expectedReward))).to.eq.BN(
      new BN(finalBalance).add(totalGasCost)
    )
