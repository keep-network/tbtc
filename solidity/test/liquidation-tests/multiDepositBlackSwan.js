...
describe("Integration -- Black-swan", async function () {
  ...
  before(async () => {
    ...
    for (let i = 0; i < depositNumber; i++) {
      ...
      const ecdsaKeep = await ECDSAKeepStub.new()
  ...
  it("Liquidate Deposits", async () => {
    ...
    for(let i = 0; i < deposits.length; i++) {
      ...
      if(available.gt(totalRequirement) || available.eq(totalRequirement)){
    ...
    const expectedReward = new BN(minSufficientlyCollateralized).mul(new BN(liquidatedIndex))
