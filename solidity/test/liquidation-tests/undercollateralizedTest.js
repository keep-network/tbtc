...
describe("Undercollateralized", async function () {
  ...
  before(async () => {
    ...
    const undercollateralized = await tbtcSystem.getSeverelyUndercollateralizedThresholdPercent.call()

    minUndercollateralized = lotSize.mul(satwei).mul(undercollateralized).div(new BN(100))

    await ecdsaKeepStub.setBondAmount(minUndercollateralized)
    await ecdsaKeepStub.send(minUndercollateralized)
  ...
  describe("undercollateralized", async () => {
    ...
    it("completes liquidation", async () => {

      await tbtcToken.resetAllowance(testDeposit.address, lotSizeTbtc, { from: auctionBuyer })
