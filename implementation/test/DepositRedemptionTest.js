...
describe('DepositRedemption', async function() {
  ...
  describe('getOwnerRedemptionTbtcRequirement', async () => {
    ...
    it('returns signerFee if we are pre-term and owner is not FRT holder', async () => {
      const tbtcOwed = await testDeposit.getOwnerRedemptionTbtcRequirement.call(owner)
    ...
    it('returns zero if deposit is pre-term, owner is FRT holder and signer fee is escrowed', async () => {
      await feeRebateToken.transferFrom(accounts[4], owner, tdtId, { from: accounts[4] })
  ...
  describe('getRedemptionTbtcRequirement', async () => {
    ...
    it('returns full TBTC if we are pre term and we are at COURTESY_CALL', async () => {
      ...
      const tbtcOwed = await testDeposit.getRedemptionTbtcRequirement.call(owner)
    ...
    it('returns full TBTC if we are at-term and caller is not TDT owner', async () => {
      ...
      await tbtcDepositToken.transferFrom(tdtHolder, accounts[1], tdtId, { from: owner })
  ...
  describe('performRedemptionTBTCTransfers', async () => {
    ...
    beforeEach(async () => {
      ...
      await tbtcToken.resetAllowance(testDeposit.address, depositValue, { from: owner })
    })
    ...
    it('escrows signerFee if deposit is pre-term, redeemer is FRT holder and signerFee is not escrowed', async () => {
      ...
      await tbtcToken.resetAllowance(testDeposit.address, signerFee, { from: owner })
      block = await web3.eth.getBlock('latest')
    ...
    it('burns 1 TBTC if deposit is in COURTESY_CALL and TDT owner is the Vending Machine', async () => {
      await tbtcDepositToken.transferFrom(owner, vendingMachine.address, tdtId, { from: owner })
      block = await web3.eth.getBlock('latest')
      await testDeposit.setState(states.COURTESY_CALL)
    ...
    it('escrows correct fee if deposit is pre-term, owner is TDT holder and signer fee is partially escrowed', async () => {
      ...
      await tbtcToken.resetAllowance(testDeposit.address, expectedFee, { from: owner })
      block = await web3.eth.getBlock('latest')
    ...
    it('burns 1 TBTC if deposit is at-term and Deposit Token owner is Vending Machine', async () => {
      await increaseTime(depositTerm.toNumber())
      await tbtcDepositToken.transferFrom(tdtHolder, vendingMachine.address, tdtId, { from: owner })
      await tbtcToken.resetBalance(depositValue, { from: owner })
      await tbtcToken.resetAllowance(testDeposit.address, depositValue, { from: owner })
      block = await web3.eth.getBlock('latest')
  ...
  describe('requestRedemption', async () => {
    ...
    beforeEach(async () => {
      ...
      await tbtcToken.resetAllowance(testDeposit.address, requiredBalance, { from: owner })
    ...
    it('updates state successfully and fires a RedemptionRequested event', async () => {
      ...
      await testDeposit.requestRedemption('0x1111111100000000', redeemerOutputScript, { from: owner })
    ...
    it('reverts if not in Active or Courtesy', async () => {
      ...
      await expectRevert(
        testDeposit.requestRedemption('0x1111111100000000', '0x' + '33'.repeat(20), { from: owner }),
    ...
    it('reverts if the fee is low', async () => {
      await expectRevert(
        testDeposit.requestRedemption('0x0011111111111111', '0x' + '33'.repeat(20), { from: owner }),
        'Fee is too low'
      )
    })
    ...
    it('reverts if the caller is not the deposit owner', async () => {
      ...
      await tbtcDepositToken.transferFrom(tdtHolder, frtHolder, tdtId, { from: owner })
  ...
  describe('approveDigest', async () => {
    ...
    it('calls keep for signing', async () => {
      ...
      expect(eventList[0].returnValues.digest, 'incorrect digest in emitted event').to.equal(digest)
    })
  ...
  describe('increaseRedemptionFee', async () => {
    ...
    beforeEach(async () => {
      ...
      await testDeposit.setRequestInfo(ZERO_ADDRESS, redeemerOutputScript, initialFee, withdrawalRequestTime, prevSighash)
      await ecdsaKeepStub.setSuccess(true)
    })
    ...
    it('reverts if the increase fee timer has not elapsed', async () => {
      const block = await web3.eth.getBlock('latest')
      await testDeposit.setRequestInfo(ZERO_ADDRESS, redeemerOutputScript, initialFee, block.timestamp, prevSighash)
    ...
    it('reverts if the previous sighash was not the latest approved', async () => {
      await testDeposit.setRequestInfo(ZERO_ADDRESS, redeemerOutputScript, initialFee, withdrawalRequestTime, keepPubkeyX)
  ...
  describe('notifySignatureTimeout', async () => {
    ...
    it('reverts if the signature timeout has not elapsed', async () => {
      await testDeposit.setRequestInfo(ZERO_ADDRESS, ZERO_ADDRESS, 0, withdrawalRequestTime + timer + 1, bytes32zero)
      await expectRevert(
        testDeposit.notifySignatureTimeout(),
  ...
  describe('notifyRedemptionProofTimeout', async () => {
    ...
    it('reverts if the proof timeout has not elapsed', async () => {
      await testDeposit.setRequestInfo(ZERO_ADDRESS, ZERO_ADDRESS, 0, withdrawalRequestTime * 5, bytes32zero)
