...
contract('DepositFactory', (accounts) => {
  ...
  describe('createDeposit()', async () => {
    it('creates new clone instances', async () => {
      ...
      clone1 = await factory.getAddress.call(0)
      clone2 = await factory.getAddress.call(1)
      try {
        await factory.getAddress.call(2)
        assert(false, 'should be out of bounds here')
      } catch (e) {
        assert.include(e.message, 'index out of bounds')
      }
    })
  })
  ...
  describe('Deposit clones have unique state', async () => {
    it('deposit 2 is not affected by state changes to deposit 1', async () => {
      ...
      expect(deposit1state, 'Deposit 1 should be in AWAITING_BTC_FUNDING_PROOF').to.eq.BN(2)
      expect(deposit2state, 'Deposit 2 should be in AWAITING_SIGNER_SETUP').to.eq.BN(1)
    })
  })
  ...
  describe('Deposit clones have unique state', async () => {
    it('deposit 1 is not affected by state changes to deposit 2', async () => {
      ...
      expect(deposit1state, 'Deposit 1 should be in AWAITING_BTC_FUNDING_PROOF').to.eq.BN(2)
      expect(deposit2state, 'Deposit 2 should be in ACTIVE').to.eq.BN(5)
    })
  })
})
