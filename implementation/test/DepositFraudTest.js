...
contract('Deposit', (accounts) => {
  ...
  describe('provideSPVFraudProof', async () => {
    ...
    it('executes', async () => {
      await testInstance.provideSPVFraudProof(_version, _txInputVector, _txOutputVector, _txLocktime, proof, index, headerChain)
    })
    ...
    it('reverts if in the funding flow', async () => {
      ...
      await expectThrow(
        testInstance.provideSPVFraudProof('0x00', '0x00', '0x00', '0x00', '0x00', 0, '0x00'),
    ...
    it('reverts if it can\'t verify the Deposit UTXO was consumed', async () => {
      ...
      await expectThrow(
        testInstance.provideSPVFraudProof(_version, _txInputVector, _txOutputVector, _txLocktime, proof, index, headerChain),
