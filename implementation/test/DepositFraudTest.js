...
describe('DepositFraud', async function() {
  ...
  describe('provideFundingECDSAFraudProof', async () => {
    ...
    it('updates to awaiting fraud funding proof and logs FraudDuringSetup if the timer has not elapsed', async () => {
      ...
      await testDeposit.provideFundingECDSAFraudProof(0, bytes32zero, bytes32zero, bytes32zero, '0x00')
      ...
      expect(actualFundingProofTimerStart, 'fundingProofTimerStart did not increase').to.be.bignumber.greaterThan(new BN(fundingProofTimerStart))
  ...
  describe('notifyFraudFundingTimeout', async () => {
    ...
    it('updates state to setup failed, logs SetupFailed, and deconstes state', async () => {
      ...
      expect(signingGroupRequestedAt, 'signingGroupRequestedAt not deconsted').to.be.bignumber.equal(new BN(0))

      const fundingProofTimerStart = await testDeposit.getFundingProofTimerStart.call()
      expect(fundingProofTimerStart, 'fundingProofTimerStart not deconsted').to.be.bignumber.equal(new BN(0))
  ...
  describe('provideECDSAFraudProof', async () => {
    ...
    it('executes', async () => {
      await testDeposit.provideECDSAFraudProof(0, bytes32zero, bytes32zero, bytes32zero, '0x00')
    })
    ...
    it('reverts if in the funding flow', async () => {
      ...
      await expectRevert(
        testDeposit.provideECDSAFraudProof(0, bytes32zero, bytes32zero, bytes32zero, '0x00'),
  ...
  describe('validateRedeemerNotPaid', async () => {
    ...
    beforeEach(async () => {
      ...
      await testDeposit.setRequestInfo(ZERO_ADDRESS, redeemerOutputScript, 2424, 0, bytes32zero)
    })
    ...
    it('returns true if redeemer is not paid', async () => {
      await testDeposit.setRequestInfo(ZERO_ADDRESS, '0x' + '0'.repeat(20), 2424, 0, bytes32zero)
    ...
    it('returns true if there is no witness flag', async () => {
      ...
      await testDeposit.setRequestInfo(ZERO_ADDRESS, newPKH, 2424, 0, bytes32zero)
