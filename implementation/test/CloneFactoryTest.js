...
describe('CloneFactory', async function() {
  ...
  describe('createClone()', async () => {
    it('creates new clone', async () => {
      ...
      const eventList = await cloneFactory.getPastEvents('ContractCloneCreated', { fromBlock: blockNumber, toBlock: 'latest' })
      ...
      expect(web3.utils.isAddress(cloneAddress), 'cloneAddress should be an address').to.be.true

      const cloneInstance = await Dummy.at(cloneAddress)
      const cloneInstanceState = await cloneInstance.getState()
      expect(cloneInstanceState, 'State should be unset').to.be.bignumber.equal(new BN('0'))
    })
  ...
  describe('isClone()', async () => {
    it('correctly checks if address is a clone', async () => {
      ...
      const checkClone = await cloneFactory.isClone_exposed.call(masterContract.address, cloneAddress)
