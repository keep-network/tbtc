...
`TBTCSystemStub` contract for `tBTC`.

#### Prepare

1. Update `config.toml` with `TBTCSystemStub` contract address.
...
1. Execute the `1_create_new_deposit.js` script with truffle:
   ```sh
   truffle exec demo/1_create_new_deposit.js
   ```

1. Execute the `2_get_public_key.js` script with truffle:
   ```sh
   truffle exec demo/2_get_public_key.js
   ```

1. Send a funding bitcoin witness transaction to the address generated by the keep.

1. Execute the `3_provide_funding_proof.js` script with truffle with funding
transaction ID (e.g. `4e3388fad9732d3c099fa95a98c9d2dd062e4a8cf7ad03f647eaccc478a79068`)
and required headers length (`0`):
   ```sh
   truffle exec demo/3_provide_funding_proof.js <TX_ID> <HEADERS_LENGTH>
   ```
...
- From running `2_get_public_key.js` script:
    ```
    Using network 'development'.
