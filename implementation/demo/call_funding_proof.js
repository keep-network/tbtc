const fs = require('fs');
const BN = require('bn.js')

var Deposit = artifacts.require("./Deposit.sol");
var KeepMediator = artifacts.require("./KeepBridge.sol");

// const keepRegistry = "0xc84Ba13B9a6f3506557dF7A78A9a63d87e1f1FcC"; // KeepRegistry contract address
// const TBTCSystem = "0x3790354Be419094B6af6c80b1dB6521c82B75192"; //TBTC System contract address

// const _bitcoinTx = '';
// const txID = '0xd8973d0405cda733f60f92362eec3ae02d4fd52896153f5c8a9f6089862368cb'
// const _version = '0x01000000'
// const _vin = `0x01179137fe810d6712b6b00c196c1b76a93240f396c53d15f4cd523995dad6f6880000000000ffffffff`
// const _vout = '0x02581b000000000000160014d849b1e1cede2ac7d7188cf8700e97d6975c91c4e8030000000000001976a914d849b1e1cede2ac7d7188cf8700e97d6975c91c488ac'
// const _fundingOutputIndex = 0;
// const _locktime = '0x00000000'
// const _merkleProof = '';
// const _merkleValid = '0xcb68238689609f8a5c3f159628d54f2de03aec2e36920ff633a7cd05043d97d800000000'
// const _index = 71;
// const _bitcoinHeaders = '';
const _signerX = '8896955d043b5a43957b21901f2cce9f0bfb484531b03ad6cd3153e45e73ee2e'
const _signerY = 'f687f923b5896a409cb7e2b5ae456f61ac61862305c6ec86bd7421b5bce115e0'
// const _value = 7000;
// const _outValueBytes = '0x581b000000000000'
module.exports = async function () {
    let deposit = await Deposit.deployed();
    let TXID = process.argv[4]
    let headerLen = process.argv[5]
    let keepBridge, inputs
    let version, vin, vout, locktime, merkleProof, index, fundingOutputIndex, bitcoinHeaders

    async function initContracts() {
        deposit = await Deposit.deployed();
        keepBridge = await KeepMediator.deployed();
    }

    async function callFundingProof() {
        console.log('Call provideBTCFundingProofV2');
        let result = await deposit.provideBTCFundingProofV2(
            version, 
            vin, 
            vout, 
            locktime, 
            merkleProof,
            index,
            fundingOutputIndex,
            bitcoinHeaders
        ).catch((err) => {
            console.log(`call failed: ${err}`);
        });

        console.log('funding call tx: ' + result.tx);
    }

    async function callretrieveSignerPubkey(){
        console.log('Call retrieveSignerPubkey');
        let key = await deposit.retrieveSignerPubkey()
        // .catch((err) => {
        //     console.log(`call failed: ${err}`);
        // });
        console.log(key);
    }

    async function callSetKeepPublicKey(){
        console.log('Call SetKeepPublicKey');
        await keepBridge.setKeepPublicKey('0x'+_signerX+_signerY).catch((err) => {
            console.log(`call failed: ${err}`);
         });
        
    }

    async function callMerkle() {
        if(TXID == undefined || TXID.length < 64){
            console.log('missing TxId argument \nexiting...');
            process.exit();
        } 
    
        const spawn = require("child_process").spawn;
        await spawn('python3',["merkle.py", TXID, headerLen]);
    }

    async function getData(){
        let rawdata = fs.readFileSync('spv_out.json');  
        inputs = JSON.parse(rawdata);
        process.stdout.write('waiting for json');
       
        let i=0;
        while(inputs.txid !== TXID){
            if(i > 15){
                console.log("json didn't update, check merkle.py")
                process.exit();
            }
            process.stdout.write('.');
            await sleep(300);
            rawdata = fs.readFileSync('spv_out.json'); 
            inputs = JSON.parse(rawdata);
            i++
        }
        console.log("");
        console.log(inputs);  
    }

    async function parseData(){
        tx = inputs.tx;
        merkleProof = '0x'+inputs.proof;
        index = inputs.index;
        bitcoinHeaders = '0x'+inputs.chain;
        version = await getVersion(tx);
        vin = await getVin(tx);
        vout = await getVout(tx);
        locktime =  await getLocktime(tx);
        fundingOutputIndex = await getOutIndex(tx);

        console.log(version, vin, vout, locktime, merkleProof, index, fundingOutputIndex, bitcoinHeaders );
    }

    async function getVersion(tx){ return '0x'+tx.slice(0,8)}
    async function getLocktime(tx){ return '0x'+ tx.slice(tx.length - 8)}

    async function getVin(tx){
        let incount = parseInt('0x'+tx.slice(12,14))
        if(incount > 1){
            console.log('Not yet supporting multiple inputs. Exiting...');
            process.exit();
        }
        return '0x'+tx.slice(12,96)
    }

    async function getOutIndex(tx){
        return 0
    }

    function sleep(ms) {
        return new Promise(resolve => setTimeout(resolve, ms));
    }

    async function getVout(tx){
        let outcount = extractNumOutputs(tx);
        let outBytes = extractNumOutputsBytes(tx)
        for(let i=0;i<outcount;i++){
            outBytes = outBytes + (extractOutputAtIndex(tx, i));
        }
        return outBytes
    }

    function getOutLen(b){
        if (b == '2200'){ return 43 * 2; }
        if (b == '1600'){ return 31 * 2; }
        if (b == '1976'){ return 34 * 2; }
        if (b == '17a9'){ return 32 * 2; }
        if (b.slice(2, 3) == "6a") {
            let pushlen = parseInt('0x'+_b.slice(0, 3));
            if(pushLen < 76, "Multi-byte pushes not supported"){process.exit()}
            return 9 + pushLen;
        }
        println("Unable to determine output length");
        process.exit();
    }

    function extractOutputAtIndex(b, index){
        if(index > extractNumOutputs(b)){
            console.log("Index more than number of outputs")
            process.exit()
        } 
        let offset = findNumOutputs(b) + 2;
        let len= getOutLen(b.slice(offset + 16, offset + 16 + 4));
        for (let i = 0; i < index; i++) {
            offset = offset + len;
            len = getOutLen(b.slice(offset + 16, offset + 16 + 4));
        }
        return b.slice(offset, offset + len);
    }

    function extractNumOutputs(b){
        let offset = findNumOutputs(b);
        let n = parseInt('0x'+b.slice(offset, offset + 2));
        if(n > 0xfd){
            console.log("VarInts not supported")
            process.exit()
        } 
        return n;
    }

    function findNumOutputs(b){
        //Return 98 for now given single P2WPHK INPUT
        //resturns lexngth of end of inputs when multiple inputs are added
        return 96;
    }

    function extractNumOutputsBytes(b){
        let offset = findNumOutputs(b);
        return '0x'+b.slice(offset, offset + 2);
    }

    // Changes the endianness of a string
    function reverseEndianness(_b) {
        let result = [];
        let len = _b.length - 2;
        while (len >= 0) {
          result.push(_b.substr(len, 2));
          len -= 2;
        }
        return result.join('');
    }

    /// Extracts the target from a block header
    function extractTarget(_b) {
        let _m = _b.slice(72*2, ((72*2) + (3*2)));
        let _e = _b.slice(75*2, (75*2) + (1*2));
        let _mantissa = new BN(reverseEndianness(_m),16);
        let _exponent = new BN(parseInt("0x"+_e));
        return _mantissa.mul(new BN(256).pow(_exponent));
    }

    // Calculate difficulty from the difficulty 1 target and current target
    function calculateDifficulty(_target) {
        // Difficulty 1 calculated from 0x1d00ffff
        let _difficulty1Target = new BN('26959535291011309493156476344723991336010898738574164086137773096960',10);
        return _difficulty1Target.div(new BN(_target));
    }
    
    function getCurrentDifficulty(){
        
    }
    //await initContracts();
        //await callMerkle();
    await getData();
    await parseData();
    //await callSetKeepPublicKey()
    //await callretrieveSignerPubkey();
    //await callFundingProof();
    //await extractTarget('0x'+bitcoinHeaders)
    console.log(calculateDifficulty(extractTarget('0x'+bitcoinHeaders)))
    process.exit();

}
