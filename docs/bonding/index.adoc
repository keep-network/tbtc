...
Overly overcollateralized Signers have the ability to safely rebalance their
bonds back to {totalcollateral}.
 
// TODO insert a little historical analysis for a decent starting number
// How did this relate to pre-liquidation? In this case, we have a
// Signer overcollateralizing by an amount which is more than the minimum, this
// is OK to happen, but is capital inefficient for them. Why not allow them to
// rebalance anytime they want (contrary to having one more parameter to set), 
// as long as their collateral is >=150% of btc value?
...
position before it becomes severely undercollateralized, or rebalance their
bonds above the pre-liquidation threshold, as all signers risk
losing funds in the liquidation process.
// comment(Georgios): Once the collateral <125%, we go into preliq, then we have
// to top up above 125%. Should we instead top-up back to 150%? I think as a
// spec we should say "Signers MUST top up above 125% to avoid liquidation, but
// SHOULD consider topping up back to 150% or more, to avoid going through the
// courtesy call on further price fluctiation in a short amount of time "
...
//  James comment from issue: 
//  We start by offering 75% of the signer' bonds for LOT_SIZE TBTC 
//  (just enough to bring supply peg back in line). 
//  We increase the percentage on offer smoothly over AUCTION_PERIOD seconds in increments of 1%.
