...
groups are not free to create, deposit requests include a small payment in the
host chain's native token to cover the creation of the signing group.
...
by the system, the deposit is marked as active or qualified and can be used to
mint TBTC token in the vending machine (see
<<{root-prefix}minting/index.adoc#vending-machine>> for more). If a deposit
proof is not received within a given timeout window, the signing group will
disband and the system will mark the deposit closed.
