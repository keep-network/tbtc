...
A simplified view of the deposit creation, minting, and redemption flows is in
this diagram:

[mermaid,basic-deposit-lifecycle,svg,theme=neutral]
.....
sequenceDiagram
   ...
   participant redeemer as Redeemer

   depositor ->> tbtc: Request deposit creation
   tbtc ->> deposit: Create deposit
   tbtc ->> depositor: Mint and assign deposit TDT
   depositor ->> deposit: Submit funding proof
   deposit ->> deposit: Move state to Qualified
   depositor ->> vending: Reassign TDT ownership to Vending Machine
   vending ->> depositor: Mint and transfer TBTC: lot size - signer fee
   vending ->> deposit: Mint and escrow TBTC: signer fee
   deposit ->> depositor: Mint and assign Fee Rebate Token

   redeemer ->> vending: Send `lot size` TBTC and TDT id
   vending ->> redeemer: Reassign TDT ownership to redeemer
   redeemer ->> deposit: Request redemption, send signer fee
   deposit ->> deposit: Produce redemption signature
   redeemer ->> deposit: Prove redemption transaction on BTC
   deposit ->> deposit: Move state to Redeemed, release signer bonds
.....

Note that the above diagram skips a few additional possibilities explained below
in this spec, including how pre- vs at-term deposits behave differently,
liquidation-related changes, as well as how a depositor might keep their TDT
without minting TBTC.
...
qualified by a fixed work requirement proven via an
<<appendix/spv/index.adoc#spv,SPV proof>>, set at {tx-proof-difficulty-factor}
of accumulated work.
...
The holder of a qualified TDT may exchange that TDT for newly minted TBTC
equivalent to the deposit's lot size (for example, {tbtc-lot-size}), less the
requisite signing fee (for example, {signer-fee}). To reflect the reduced
...
Any TDT held by the vending machine can be obtained for its lot size in TBTC (in
the above instance, the deposit could be obtained for {tbtc-lot-size}). The
vending machine MUST burn any TBTC it receives, in any case where it can receive
TBTC.
