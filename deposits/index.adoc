...
// TODO: why is TBTC withheld

Once a deposit has been made and funded, the corresponding TBTC are minted. All
minted TBTC are immediately issued to the funder, who is now the beneficiary of
a funded _Deposit_. Some TBTC is withheld to ensure eventual payment of
custodial fees. Beneficary status is transferable (in Ethereum this is
implemented as an ERC721 compatible non-fungible token). Should the _Deposit_
resolve normally, the witheld TBTC will be returned to the current beneficiary
along with a small additional payment. In this way the beneficiary NFT
functions as a zero-coupon bond issued by the signing group upon funding.
